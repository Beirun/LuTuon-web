    ...
    <div class="w-full h-1/10 flex justify-between items-center px-10 shadow-xs">
      ...
      <div class="flex w-full h-full justify-end items-center gap-5">
        ...
        <Button variant="outline"
          class="h-12 w-12 shadow-sm shadow-black/20 active:brightness-90 hover:brightness-95 duration-300 cursor-pointer">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child>
            <Button variant="outline"
              ...
              <component :is="Triangle" height="15"
                class="rotate-180 fill-[#3A3A3A] text-[#3A3A3A] dark:text-[#e8e8e8] dark:fill-[#e8e8e8]" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" class="sm:w-40">
            <DropdownMenuItem @click="handleLogout" class="cursor-pointer flex items-center justify-between text-base">
              <span>Logout</span>
              <LogOut class="size-5" />
            </DropdownMenuItem>
          </DropdownMenuContent>
        ...
        <div class="flex gap-4 items-center">
                ...
                <div v-for="item in items" class="sm:my-10 my-4 flex flex-col w-full">
                  <a @click="handleSwitch(item.title.toLowerCase())" :class="[
                    ...
                    <component :is="item.icon" class="size-6"
                      :class="activeItem === item.title.toLowerCase() ? 'text-yellow-400 dark:text-yellow-300' : 'text-[#515151]'" />
          ...
          <p class="text-xl sm:text-2xl font-bold">
            ...
            <span class="block sm:hidden">
              ...
              {{ activeItem === 'account overview' ? 'Account Overview' : activeItem === 'personal information' ?
                'Personal Information' : 'Security' }}
            </span>
            <!-- end change header when mobile -->

            <span class="hidden sm:block">Account Center</span> <!-- header when laptop -->
          </p>
        </div>
        ...
        <div class="sm:w-2/3 w-full flex flex-col gap-5">
          ...
          <div v-if="activeItem !== 'security'" class="w-full border border-border rounded-xl">
            <div class="flex justify-between border-b p-3 items-center">
              ...
              <Button v-if="activeItem === 'account overview'" @click="handleSwitch('personal information')"
                class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-[#F8C600]">Manage</Button>
              <Button v-if="activeItem === 'personal information' && !edit" @click="handleEdit"
                class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-[#F8C600]">Edit</Button>
              <div v-if="edit" class="flex gap-5">
                <Button :disabled="auth.isLoading" @click="handleCancel"
                  class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-gray-500 dark:text-gray-300 underline">
                  Cancel
                </Button>
                <Button :disabled="auth.isLoading" @click="handleSave" class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-[#F8C600]">
                  Save
                </Button>
              </div>
            </div>
          ...
          <div class="w-full border border-border rounded-xl">
            <div class="flex justify-between border-b p-3 items-center">
              ...
              <Button v-if="activeItem === 'account overview'" @click="handleSwitch('security')"
                class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-[#F8C600]">
                Manage
              </Button>
              <Button v-if="activeItem === 'security' && !managePasswordEdit" @click="handlePasswordEdit"
                class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-[#F8C600]">
                Edit
              </Button>
              <Button v-if="activeItem === 'personal information'"
              ...
              <div v-if="managePasswordEdit" class="flex gap-5">
                ...
                <Button :disabled="auth.isLoading" @click="handlePasswordCancel"
                  class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none dark:text-gray-300 text-gray-500 underline">
                  
                  Cancel
                </Button>
                <Button :disabled="auth.isLoading" @click="handlePasswordSave" class="bg-transparent p-0 hover:bg-transparent cursor-pointer shadow-none text-[#F8C600]">
                  Save
                </Button>
              </div>
              ...
              <Table v-if="activeItem === 'account overview' || activeItem === 'security'" class="my-5">
                <TableBody>
                  <TableRow class="flex justify-between px-5 hover:bg-transparent">
                    ...
                    <TableCell class="text-foreground text-center w-2/3 sm:w-1/2 flex items-center justify-end">
                      ...
                      <div v-else
                        ...
                        <input :type="showOldPassword ? 'text' : 'password'"
                          ...
                          class="text-right w-full focus:outline-none px-3" placeholder="Old Password" />
                        <component v-if="!showOldPassword" @click="TogglePassword('old')" :is="EyeClosed"
                          class=" text-[#3A3A3A] cursor-pointer" />
                        <component v-else @click="TogglePassword('old')" :is="Eye"
                          class=" text-[#3A3A3A] cursor-pointer" />
                      </div>
                    </TableCell>
                  </TableRow>
                  <TableRow v-if="managePasswordEdit" class="flex justify-between px-5 hover:bg-transparent">
                    <TableCell class="h-[6vh] text-foreground text-center flex items-center"></TableCell>
                    <TableCell class="text-foreground text-center w-2/3 sm:w-1/2 flex items-center justify-end">
                      <div
                        class="flex justify-end items-center px-2 w-full shadow-none focus:outline-none border rounded-lg h-full bg-popover text-popover-foreground">
                        ...
                        <input :type="showNewPassword ? 'text' : 'password'"
                          ...
                          class="text-right w-full focus:outline-none px-3" placeholder="New Password" />
                        <component v-if="!showNewPassword" @click="TogglePassword('new')" :is="EyeClosed"
                          class=" text-[#3A3A3A] cursor-pointer" />
                        <component v-else @click="TogglePassword('new')" :is="Eye"
                          class=" text-[#3A3A3A] cursor-pointer" />
                      </div>
                    </TableCell>
                  </TableRow>
                    ...
                    <TableCell class="text-foreground text-center  w-2/3 sm:w-1/2 flex items-center justify-end">
                      <div
                        ...
                        <input :type="showConfirmPassword ? 'text' : 'password'"
                          ...
                          class="text-right w-full focus:outline-none px-3" placeholder="Confirm Password" />
                        <component v-if="!showConfirmPassword" @click="TogglePassword('confirm')" :is="EyeClosed"
                          class=" text-[#3A3A3A] cursor-pointer" />
                        <component v-else @click="TogglePassword('confirm')" :is="Eye"
                          class=" text-[#3A3A3A] cursor-pointer" />
                      </div>
                    </TableCell>
                  </TableRow>
